    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-6">
        <NeoCard variant="obsidian" className="max-w-md w-full p-8 border-red-900 text-center">
          ...
          <button onClick={() => router.push('/dashboard')} className="text-gray-400 hover:text-white underline">
            Back to Dashboard
          </button>
        </NeoCard>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-8">
            <NeoCard variant="obsidian" className="p-8">
              <h2 className="text-sm font-bold text-cyan-400 uppercase tracking-wider mb-6">Current Plan</h2>
              <div className="mb-8">
                <span className="text-3xl font-bold text-white">{subscription?.tier}</span>
                <span className="ml-2 text-gray-500 text-sm">/ {subscription?.billingInterval || 'forever'}</span>
              </div>
            ...
            {!isFree && (
              <div className="bg-red-900/10 border border-red-900/30 p-6 rounded-lg">
                ...
                <p className="text-sm text-gray-500 mb-6">
                  Cancelling your subscription will restrict your account to the Free tier features at the end of your current billing period.
                </p>
                <button
                ...
                >
                  {cancelLoading ? 'Cancelling...' : subscription?.cancelAtPeriodEnd ? 'Cancellation Scheduled' : 'Cancel Subscription'}
                </button>
          ...
          <div>
            <NeoCard variant="obsidian" className="p-8 h-full">
              <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-6">Plan Entitlements</h3>
              <ul className="space-y-4">
                {Object.entries(subscription?.plan?.features || {}).map(([key, feature]: [string, any]) => (
                  <li key={key} className="border-b border-gray-800 pb-4 last:border-0">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-cyan-400 font-mono">
                        {feature.limit === -1 ? 'Unlimited' : feature.limit === 0 ? 'Locked' : feature.limit}
                      </span>
                    </div>
                    {feature.limit > 0 && (
                      <div className="w-full bg-gray-900 h-1 rounded-full overflow-hidden mt-2">
                        <div 
                          ...
                          style={{ width: `${Math.min(100, (feature.used / feature.limit) * 100)}%` }}
                        ></div>
                      </div>
                    )}
                  </li>
