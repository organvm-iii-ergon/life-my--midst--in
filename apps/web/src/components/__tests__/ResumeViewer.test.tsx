...
describe("ResumeViewer", () => {
  ...
  it("displays persona header with role vector", () => {
    render(
      <ResumeViewer entries={mockEntries} persona={mockPersona} />
    );
  ...
  it("shows batch generation UI when available", () => {
    ...
    expect(
      screen.getByRole("button", { name: /generate all|batch/i })
    ).toBeInTheDocument();
  });
  ...
  it("handles empty entries gracefully", () => {
    ...
    expect(
      screen.getByText(/no entries|empty|create/i)
    ).toBeInTheDocument();
  });
